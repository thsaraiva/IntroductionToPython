...
nom_count_dict = {}
for _, nominees in nominated.items():
...
for year, winner in winners.items():
    for champs in winner:
        win_count_dict[champs] = win_count_dict.get(champs, 0) + 1
print("win_count_dict = {}".format(win_count_dict))

# 2. Provide a list with the name(s) of the director(s) with the most Oscar wins. We are asking for a list because
# there could be more than 1 director tied for the most Oscar wins.

max_win_numbers = max(win_count_dict.values())
most_win_director = [director for director, wins in win_count_dict.items() if wins == max_win_numbers]
